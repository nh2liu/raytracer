...
RGBUnit Camera::color(const Ray &r, const vector<RenderObject *> &objects,
    ...
    if (!lowest_obj || bounces == max_bounces_) {
        ...
        if (m) {
            ...
            return attenuation * color(Ray(poi, newVec), objects, bounces + 1);
        } else {
    ...
    for (uint32_t j = 0; j < y_res_; j++) {
        for (uint32_t i = 0; i < x_res_; i++) {
            ...
            for (uint32_t alias = 0; alias <= aliasing_its_; alias++) {
                ...
                const auto &objects = scene_manager.getObjects(r);
                pxl += color(r, objects);
            }
            progress_counter++;
