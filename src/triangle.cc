...
float Triangle::intersect(const Ray &r, const float tMin,
    ...
    if (t > tMin && t < tMax) {
        ...
        float alpha =
            (cross(poi - points[1], poi - points[2]).mag()) / (2 * area);
        float beta =
            (cross(poi - points[2], poi - points[0]).mag()) / (2 * area);
...
Vec3 Triangle::normal(const Vec3 poi) const { return norm; }
